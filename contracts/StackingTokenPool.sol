...
contract StackingTokenPool {
    address public owner;
    WibuNFT public wibuNFT;
    WibuToken public wibuToken;

    mapping(address => ListedToken) public stackings;
    ...
    function StackingToken(uint256 _amount) public returns (uint256) {
        ...
        stackings[msg.sender] = ListedToken(
            _amount,
            payable(msg.sender),
            block.timestamp
        );
        emit TokenStackedSuccess(_amount, msg.sender, block.timestamp);
    ...
    function claimReward() public {
        require(
            block.timestamp >= stackings[msg.sender].stackingTime + 1,
